...
### `tqp geometry profile`, `tqp fuzz retrieval`, and `tqp fuzz replay`
...
`tqp fuzz replay fuzz-run/cases/case-000000 --format json` validates every payload checksum, bundle schema, version, frozen geometry hash, index hash, and recorded evidence before opening the stored immutable index in a temporary location.
It recomputes exact and compressed top-k results without refitting geometry, then requires discrete outcomes to match exactly and floating values to fit the case's recorded tolerances.
Missing, modified, incomplete, or incompatible bundles fail closed without evaluation.

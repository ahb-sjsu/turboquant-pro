...
> Test counts are **pytest-collected item counts** (parametrized cases count individually), snapshotted at each release — not the raw `def test_` function count, which is lower. The table jumps from v1.4.3 to **1.8.0** (the certification-platform release); the current **stable** PyPI release is **1.9.1** (the 2.0.0 pre-release line — a1, a2 published; a3 on `master` — is in [`CHANGELOG.md`](../CHANGELOG.md)); the intervening 1.5.0–1.7.0 (and metadata-only 1.8.1) releases are in [`CHANGELOG.md`](../CHANGELOG.md). The [Tests badge](https://github.com/ahb-sjsu/turboquant-pro/actions) shows CI **pass/fail** status, not a count; for the exact current number run `pytest -q --co | tail -1`. Run the history benchmark: `python benchmarks/benchmark_release_history.py`.

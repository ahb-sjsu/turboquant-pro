#!/bin/bash
# Atlas-side GPU watcher for Part III run pods (adapted from GET G3c watch_jobs.sh). Every
...
while true; do
  now=$(date -u +%FT%TZ)
  active=0
  for j in $($K get jobs -l app=tqp-wo,atlas.io/role=run -o name 2>/dev/null); do
    n=${j#job.batch/}

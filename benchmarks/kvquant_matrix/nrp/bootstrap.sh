...
# KVQuant is canonically PRE-RoPE: calibrate_kvquant captures pre-RoPE k_proj keys,
# so eval MUST quantize pre-RoPE too (PREROPE=1) or centroids are applied in the wrong
# space -> collapse (post-RoPE eval gave qasper 0.59 vs fp16 22.14).
MENV[kvquant]="NOQUANT=0 CODEBOOK=kvquant KEY_BITS=4 VAL_BITS=4 GROUP=32 HOT=128 SINK=4 OUTLIER_FRAC=0.02 PREROPE=1"

...
print("LaBSE config max_position_embeddings =",
      getattr(cfg, "max_position_embeddings", "?"), flush=True)
...
labels = np.array([ln.strip() for ln in
                   open("/archive/tqp_strata/labels.txt", encoding="utf-8")])
rng = np.random.default_rng(7)
sel = np.sort(rng.choice(len(ids), size=min(SUB, len(ids)), replace=False))

conn = psycopg2.connect(dbname="atlas", user="claude")
cur = conn.cursor()
cur.execute("SELECT id, content FROM ethics_chunks WHERE id = ANY(%s)",
...
for lang, ns in sorted(per.items(), key=lambda kv: -len(kv[1])):
    a = np.array(ns)
    e = {"n_sampled": int(len(a)), "median_tokens": float(np.median(a)),
...
for cap in CAPS:
    ...
    print(f"  cap {cap}: trunc frac {f.min():.3f}-{f.max():.3f} "

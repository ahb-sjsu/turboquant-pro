...
fresh = m.encode(texts, batch_size=8, show_progress_bar=False,
...
print(f"cosine(stored, re-encoded): min={cos.min():.6f} "
      f"p01={np.percentile(cos, 1):.6f} median={np.median(cos):.6f} "
      f"mean={cos.mean():.6f}")
bad = int((cos < 0.99).sum())
print(f"rows below 0.99: {bad}/{len(cos)}")
print("PREDICATE VERIFIED" if bad == 0 else "PREDICATE SUSPECT — content may have changed")

#!/bin/bash
# 100M smoke test: 4 servers x 5 shards x 5M rows = 100M, one wave of 4.
#
# Purpose is to exercise the machinery end to end cheaply before the 10B run:
#   1. delete_job_wait actually prevents the Multi-Attach retry storm
#   2. straggler detect -> re-issue -> resume works against a live fleet
#   3. build/qcache/ref/ivf/score still produce a sane recall
# Run it with aggressive straggler settings and a short poll, because jobs
# here take tens of seconds and the production 120s poll would never observe
# one mid-flight.
...
echo "=== $(date -u +%H:%M) 100M smoke: PVCs"
for I in $(seq 0 $((N - 1))); do
  sed "s/__I__/$I/g" pvc_100m_tmpl.yaml | kubectl apply -f -
done

echo "=== $(date -u +%H:%M) straggler settings X=$STRAGGLE_X MAX=$STRAGGLE_MAX MIN_S=$STRAGGLE_MIN_S POLL=$POLL"

echo "=== $(date -u +%H:%M) build"
run_waves tqp-smoke100m-build- job_build_100m.yaml || exit 1

echo "=== $(date -u +%H:%M) query cache"
if ! job_done aqx-qcache100m; then
  delete_job_wait aqx-qcache100m
  kubectl apply -f job_qcache_100m.yaml
  wait_wave job_qcache_100m.yaml aqx-qcache100m aqx-qcache100m || exit 1
fi

echo "=== $(date -u +%H:%M) exact reference"
run_waves aqx-ref100m- job_ref_100m.yaml || exit 1

echo "=== $(date -u +%H:%M) routed IVF"
run_waves aqx-ivf100m- job_ivf_100m.yaml || exit 1

echo "=== $(date -u +%H:%M) merge + score"
delete_job_wait aqx-score100m
kubectl apply -f job_score100m.yaml
wait_wave job_score100m.yaml aqx-score100m aqx-score100m || exit 1
kubectl logs -n $NS job/aqx-score100m --tail=20 > score_100M.log 2>&1
cat score_100M.log

echo "=== $(date -u +%H:%M) 100M smoke complete"
echo DRIVER100M_DONE

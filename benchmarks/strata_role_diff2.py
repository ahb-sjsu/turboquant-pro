...
def main() -> None:
    ...
    out: dict = {
        ...
        "provenance": {
            ...
            "amendment": 1,
            "ids": a.ids.split("/")[-1],
            "ids_sha256": sha256(a.ids),
            "labels": a.labels.split("/")[-1],
            "labels_sha256": sha256(a.labels),
            "k": a.k,
            "n_min": a.n_min,
            "R": a.R,
            "seed": a.seed,
            "labels_permuted": bool(a.permute),
            "n_rows": int(n),
        },
        "areas_scored": int(m),
    ...
    def stats(fc: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        F = (
            np.bincount(cd[src[eok]] * m + cd[dst[eok]], minlength=m * m)
            .astype(np.float64)
            .reshape(m, m)
        )
        F /= max(F.sum(), 1.0)
    ...
    T_null = np.array(
        [two_means_T(taus[r] - (S - taus[r]) / (a.R - 1)) for r in range(a.R)]
    )
    ...
    out.update(
        {
            ...
            "S1p_T": T_obs,
            "S1p_p": p_T,
            "S1p_T_null_mean": float(T_null.mean()),
            "S1pb_dip": float(dip),
            "S1pb_dip_p": float(p_dip),
            "S2p_A": A_obs,
            "S2p_p": p_A,
            "S2p_A_null_mean": float(A_null.mean()),
            "S3_mi_bits_context_only": mi,
        }
    )
    json.dump(out, open(a.out, "w"), indent=2)
    print(
        f"[{a.tag}] areas={m}  mean|e_a|={out['mean_abs_excess']:.5f}  "
        f"mean e_a={out['mean_excess']:+.5f}"
    )
    print(
        f"  S1' T={T_obs:.4f} p={p_T:.4f} (null mean {T_null.mean():.4f})   "
        f"dip={dip:.4f} p={p_dip:.3f}"
    )
    print(
        f"  S2' A={A_obs:.4f} p={p_A:.4f} (null mean {A_null.mean():.4f})   "
        f"S3 MI={mi:.4f} bits"
    )
    print(f"  -> {a.out}")

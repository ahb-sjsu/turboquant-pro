...
def harness_cell():
    ...
    header = ("# === embedded harness (verbatim from benchmarks/canonical_embedding.py) ===\n"
...
def build_flagship():
    ...
    cells = [
        md(f"# Canonical SOTA embedding benchmark — {colab_badge(rel)}",
           "",
        ...
        md("## 2. Configure",
           ...
           "fast smoke pass."),
        code("DATASET    = 'glove-100-angular'",
             "QUERIES    = 2000",
...
def build_pca_truncation():
    ...
    cells = [
        md(f"# PCA rotation makes non-Matryoshka models truncatable — {colab_badge(rel)}",
           "",
        ...
        harness_cell(),
        md("## 4. Load public data"),
        code(GLOVE_LOADER.replace("C, Q, gt = load_annbench()",
                                  "C, Q, gt = load_annbench()\ndim = C.shape[1]")),
        md("## 5. Naive vs PCA truncation at matched kept-dims"),
        code("import faiss, numpy as np, pandas as pd",
...
def build_learned_codebooks():
    ...
    cells = [
        md(f"# Learned codebooks reduce quantization error — {colab_badge(rel)}",
           "",
        ...
        code(GLOVE_LOADER.replace("C, Q, gt = load_annbench()",
                                  "C, Q, gt = load_annbench()\ndim = C.shape[1]")),
        md("## 5. Learned vs fixed codebook reconstruction error",
...
def build_adc_throughput():
    ...
    cells = [
        md(f"# ADCIndex compressed-domain search throughput — {colab_badge(rel)}",
           "",
        ...
        code("rows = run_canonical(C, Q, gt, out_dim=OUT_DIM, bits=BITS, oversample=OVERSAMPLE,",
...
def build_kv_placeholder(idx, slug, title, rung, body_md):
    ...
    cells = [
        ...
        code("MODELS = ['meta-llama/Llama-3.2-1B', 'mistralai/Mistral-7B-v0.3',",
        ...
        code("# Example: reproduce one row of the matrix\n"
...
def main():
    ...
    builders = [
        ...
        lambda: build_kv_placeholder(
            "11", "kv_nf4_matrix",
            "NF4 / asymmetric-NF4 KV-cache matrix", "L2",
        ...
        lambda: build_kv_placeholder(
            "12", "kv_vs_kvquant",
            "Comparison vs KVQuant", "L4 (paper result, not independently reproduced)",

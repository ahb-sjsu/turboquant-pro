...
Combined with 3-bit scalar quantization, PCA-Matryoshka achieves 27$\times$ compression at 77\% recall@10 single-stage, rising to \textbf{99.4\%} recall@10 with standard 5$\times$ oversampling and reranking, on a 50K-vector subsample of a 2.4-million-vector cross-civilizational ethics corpus.
...
\begin{table}[h]
\centering
\caption{Recall@10 with oversampling + exact reranking on 50K production
BGE-M3 embeddings from the ethics corpus.}
\label{tab:reranking}
\begin{tabular}{lcccc}
\toprule
Method & fetch $1\times$ & $2\times$ & $5\times$ & $10\times$ \\
\midrule
TQ3 (10.5$\times$)        & 81.4\% & 97.8\% & 100.0\% & 99.8\% \\
PCA-384 + TQ3 (27.7$\times$) & 77.0\% & 96.0\% & \textbf{99.4\%} & 99.8\% \\
\bottomrule
\end{tabular}
\end{table}

With $5\times$ oversampling, PCA-384 + TQ3 achieves \textbf{99.4\% recall@10
at 27.7$\times$ compression}.  The cost of reranking 50 candidates with exact
cosine similarity is negligible compared to the initial search.  The
single-stage recall of 77.0\% is consistent with the 76.4\% reported in
Table~\ref{tab:combined} (different random query sample from the same corpus).

\subsection{Comprehensive Comparison}

Table~\ref{tab:all_methods} presents the configurations for which both
cosine similarity and recall@10 were measured.
...
PCA-384 + TQ3 reduces this to 1.4\,GB---a 27$\times$ reduction---while maintaining 99.4\% recall@10 with 5$\times$ oversampling (77\% single-stage), enabling deployment on modest hardware.
...
Combined with TurboQuant 3-bit scalar quantization, PCA-Matryoshka achieves 27$\times$ compression at 77\% recall@10 single-stage.
With standard 5$\times$ oversampling and exact reranking (measured on a 50K-vector subsample of the production corpus), recall rises to 99.4\%.
This fills a previously empty region of the compression--retrieval Pareto frontier between scalar quantization ($\leq$10$\times$) and binary/product quantization ($\geq$32$\times$).

...
We evaluate on five embedding models spanning three architectures (BERT, MPNet, BGE) and confirm the method generalizes consistently.
...
Combined with 3-bit scalar quantization and standard 5$\times$ oversampling with reranking, PCA-Matryoshka delivers 27$\times$ compression at 97.3\% recall@10 on a cross-civilizational ethics corpus (76.4\% without reranking).
We provide a comprehensive empirical comparison of 15 compression configurations and show that PCA-Matryoshka + TurboQuant strictly dominates binary quantization and product quantization on both retrieval metrics and pointwise fidelity.
Importantly, we find that cosine similarity to the original vector is \emph{not} a reliable proxy for retrieval quality at high compression: PCA-256 + TQ3 achieves lower cosine similarity (0.963) but higher recall@10 (78.2\%) than PCA-384 + TQ3 (0.979 cosine, 76.4\% recall), underscoring the need to evaluate on task-relevant retrieval metrics.
...
Section~\ref{sec:update} describes the implementation evolution from v0.5.0 to v1.0.0 with new benchmarks.
...
A key question is whether PCA-Matryoshka generalizes beyond BGE-M3.
We evaluate on four additional embedding models spanning two
architectures (BERT, MPNet) and two model families (MiniLM, BGE),
using a shared corpus of 50 diverse English sentences and PCA
truncation to the maximum dimension permitted by the sample size.

\begin{table}[h]
\centering
\caption{PCA-Matryoshka generalization across embedding models.
Na\"{\i}ve truncation destroys quality for all models; PCA rotation
consistently recovers it. TQ3 quality and learned codebook
improvement are stable across architectures.}
\label{tab:multimodel}
\begin{tabular}{lcccccc}
\toprule
Model & Dim & Na\"{\i}ve & PCA & TQ3 & Learned & $\Delta$ \\
\midrule
all-MiniLM-L6-v2   & 384 & 0.352 & 0.998 & 0.978 & 0.983 & +0.005 \\
all-mpnet-base-v2  & 768 & 0.240 & 0.998 & 0.978 & 0.983 & +0.005 \\
bge-small-en-v1.5  & 384 & 0.296 & 0.999 & 0.977 & 0.982 & +0.005 \\
bge-base-en-v1.5   & 768 & 0.234 & 0.999 & 0.978 & 0.983 & +0.005 \\
\bottomrule
\end{tabular}
\end{table}

Three findings are consistent across all models:
(1)~Na\"{\i}ve truncation yields unusable cosine similarity
(0.23--0.35), confirming that non-Matryoshka models distribute
information uniformly across dimensions.
(2)~PCA rotation recovers $>$0.997 cosine similarity in all cases,
demonstrating that the method is not specific to BGE-M3.
(3)~Learned codebooks improve over Lloyd-Max by a consistent
+0.005 across all four models, confirming that the Gaussian
approximation error is a property of the PolarQuant algorithm
...
Our primary evaluation uses BGE-M3 on a single cross-civilizational ethics corpus.
To address this, we added a multi-model evaluation
(Table~\ref{tab:multimodel}) on four models across two architectures,
confirming that PCA-Matryoshka and learned codebooks generalize consistently.
However, larger-scale evaluations on production corpora with models such as
Cohere Embed and OpenAI ada-002 remain future work.
The PCA basis is fitted on a specific corpus and may transfer imperfectly to
out-of-distribution data, though we find it generalizes well within the same
embedding model since the model's output distribution is relatively consistent.
...
\begin{table}[h]
\centering
\caption{Asymmetric K/V bit allocation (\texttt{head\_dim}=128, 8 heads).}
...
The implementation is open-source (\texttt{pip install turboquant-pro}, v1.0.0: 397 tests, 27 modules) and supports KV cache compression with asymmetric bit allocation, RoPE-aware quantization, multi-modal embedding presets, production quality monitoring, and integration with pgvector, FAISS, vLLM, Milvus, Qdrant, Weaviate, and Pinecone.

...
We compare 15 compression configurations and show that PCA-Matryoshka + TurboQuant strictly dominates binary quantization and product quantization on retrieval metrics.
...
Combined with TurboQuant 3-bit scalar quantization and standard 5$\times$ oversampling with reranking, PCA-Matryoshka achieves 27$\times$ compression at 99.4\% recall@10 on production data (77\% single-stage), filling a previously empty region of the compression--retrieval Pareto frontier between scalar quantization ($\leq$10$\times$) and binary/product quantization ($\geq$32$\times$).

...
it suggests a \textbf{deterministic, calibration-free zero-point}: since the
offset-carrying channels are identified by the model config alone
($\theta$, head dim, window), a frequency-derived zero-point schedule could
replace the per-channel calibrated $\mu$ metadata; we leave the quality
comparison against calibrated asym-NF4 to future work.

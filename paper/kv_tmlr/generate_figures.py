...
def fig_mechanism():
    ...
    fig, ax = plt.subplots(figsize=(5.0, 3.0))
    layers = ["L0", "L10", "L20"]
    qn = [0.0154, 0.0789, 0.0869]
    qu = [0.0010, 0.0190, 0.0325]
    ln = [0.0772, 0.0845, 0.0863]
    lu = [0.0243, 0.0177, 0.0192]
...
def fig_rescue():
    ...
    ax.set_title(
        "Qwen rescue: it is the codebook, not the bits\n(3-bit uniform beats 4-bit NF4 by 7x)"
    )
    ax.set_ylim(0, 50)
    save(fig, "fig_rescue")
...
def fig_asymnf4():
    ...
    for ax, title, levels, col in [
        ...
        (
            a2,
            "asym-NF4 (centre on the data)",
            data.mean() + NF4 * np.abs(data - data.mean()).max(),
            C["asym-NF4"],
        ),
    ]:

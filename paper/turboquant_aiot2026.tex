...
Foundation models are moving onto AIoT devices---smart-home hubs, industrial
gateways, robots, and field sensors---for privacy, latency, and offline
operation. On such IoT-class hardware (often a few watts and a few gigabytes of
memory shared with the OS) the binding constraint is \emph{memory and energy},
not arithmetic; and the data is increasingly multi-modal---vision, audio, and
text from on-board sensors---so models and their embedding stores must be
compressed \emph{together}, on-device. Three memory sinks dominate: the model
\emph{weights}; the

...
> **Every headline number — with its reproduction status — is in [`CLAIMS.md`](CLAIMS.md)** as a table row: claim, dataset, one-click notebook, hardware, and whether it's CPU-reproducible or GPU-experimental (27× compression, RaBitQ/OPQ comparison, 4–20× faster builds, 22% learned-codebook error reduction, KV-cache results). Test suite: ~847 pytest-collected items on master (run `pytest -q --co | tail -1` to verify; the 1.7.0 release snapshot was ~514); the badge above shows CI pass/fail status. For the code-quality posture, the tiered elegance-and-rigor **soundness audit** — every finding fixed and verified — is [`docs/soundness_audit.md`](docs/soundness_audit.md) (renders on GitHub, with diagrams; a richer standalone HTML version is [`docs/soundness_audit.html`](docs/soundness_audit.html)).
...
```
Current package:               turboquant-pro 1.8.1
...
[`v1.8.1`](https://github.com/ahb-sjsu/turboquant-pro/releases/tag/v1.8.1) is the current release — a release-metadata-only patch over [`v1.8.0`](https://github.com/ahb-sjsu/turboquant-pro/releases/tag/v1.8.0), which shipped the certification platform (no API or behavior change between them). The DOI-archived core-results artifact is [`v1.4.0`](https://github.com/ahb-sjsu/turboquant-pro/releases/tag/v1.4.0) · DOI [10.5281/zenodo.20660087](https://doi.org/10.5281/zenodo.20660087).

> ✅ **The `tqp` CLI and the certification platform ship in 1.8.0 (current release: 1.8.1).** `pip install turboquant-pro` gives you the `tqp` console script and the full `trace → plan → compress → certify → verify → replay → monitor` pipeline, plus the persisted-index/plugin certification lifecycle; add `[torch]` for `tqp trace`. See [`docs/CLI.md`](docs/CLI.md#install).
...
- **Fast & deployable.** Fused split-K decode beats decompress-then-attend **up to ~20× at 128k context** (exact to ≤3e-8; ~13× at 32k), now with a **portable Triton port** (P5) that runs the same kernels on NVIDIA without CuPy — validated exact across Turing→Ampere→Ada→Hopper, and its batched-page variant beats the CuPy RawKernel **1.9–7.9×** ([`benchmarks/RESULTS_p5_triton.md`](benchmarks/RESULTS_p5_triton.md), [`RESULTS_longcontext.md`](benchmarks/RESULTS_longcontext.md)); learned codebooks cut error **22%**; a versioned self-describing format (TQE1); production drift monitoring; cross-framework export (FAISS/Milvus/Qdrant/Weaviate/Pinecone) and a native Rust PostgreSQL extension.
- **Certified, and independently verifiable.** Every compression run can carry a **distribution-free rank floor** (`rank_certificate`: measured distortion κ + corpus concentration μ̂ → guaranteed Kendall τ ≥ 1−2μ̂; vacuous floor = per-corpus "rerank required" signal), and the **(A2) probe** selects the quantizer family against the *declared* consumer metric — the check that would have caught the v1.2.0 keys incident at calibration time. A certificate is not self-issued trust: **`tqp certify` writes the floor to a `certificate.json`, and `tqp verify` (1.8.0) lets *anyone* re-hash the inputs, re-run the math, and confirm it reproduces — the third-party trust path.** Backed by the companion theory paper ([the-angular-observer](https://github.com/ahb-sjsu/the-angular-observer)).
...
#### Fused CUDA decode
`TurboQuantKVCache.fused_decode` computes one decode step over the whole cache directly on the codes (no reconstruction) — a split-K flash-decode that **beats decompress-then-attend up to ~20× at 128k context** (exact to ≤3e-8; ~13× at 32k), merging the fp16 hot window via online softmax. A **portable Triton port** (P5) runs the same M2/M4 kernels on NVIDIA (Turing→Hopper, all exact) without CuPy; its single-launch batched-page variant beats the CuPy RawKernel **1.9–7.9×** across 2k–128k and stays exact to ~1e-7 ([`benchmarks/RESULTS_p5_triton.md`](benchmarks/RESULTS_p5_triton.md), [`RESULTS_longcontext.md`](benchmarks/RESULTS_longcontext.md)). The CuPy RawKernel remains the CUDA reference/oracle. Standalone rotate+quantize kernels fuse both passes into one tiled GEMM (eliminates ~400 MB traffic for 100K vectors @ dim 1024). Volta+ (compute 7.0).
...
- **Operator sensitivity for gates & SSMs (`operator_sensitivity`), validated on real models:** the measured (A2) boundary for the two regimes beyond attention. **Routing** is carried by the *margin*, not the magnitude — on real **Mixtral-8x7B** (top-2), 4-bit gate quantization flips 16.6% of expert sets with low-margin tokens flipping **10.7×** more than high-margin (reproducing the companion paper's ratio); on **OLMoE-1B-7B** (top-8), the margin-concentration reaches **1740×** under fine perturbation. **SSM decay** makes the *slow (long-memory) channels* fragile — on real **Mamba-790m**, 3-bit linear decay quantization collapses WikiText-2 perplexity to ~10¹⁰ while the native `A_log` (log-time-constant) basis keeps it near baseline (`quantize_decay(basis="log_tau")` — the SSM analog of NF4-for-keys). Model cards with the full evidence, reconciliations, and negative cases: [`docs/model_cards/`](docs/model_cards/); synthetic derivation in [`docs/notes/operator_sensitivity_ssm_moe.md`](docs/notes/operator_sensitivity_ssm_moe.md).
